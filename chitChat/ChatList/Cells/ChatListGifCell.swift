...
var ChatListGifCellID = "ChatListGifCellID"

class ChatListGifCell: ChatListBaseCell {
    ...
    override func setupFromUser(chatPartner: user) {
        ...
        if self.message?.fromID == message?.chatPartner {
            self.messageLbl.text = "\(chatPartner.name.capitalized) sent a gif"
        } else {
